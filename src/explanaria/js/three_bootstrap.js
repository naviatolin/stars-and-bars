...
ThreeasyEnvironment.prototype.resizeCanvasIfNecessary = function () {
  ...
  if (!this.shouldCreateCanvas) {
    // a canvas was provided externally
...
ThreeasyEnvironment.prototype.removeEventListener = function (
  event_name,
  func
) {
...
ThreeasyEnvironment.prototype.off =
  ThreeasyEnvironment.prototype.removeEventListener; //alias to match ThreeasyEnvironment.on
...
class ThreeasyRecorder extends ThreeasyEnvironment {
  ...
  record_frame() {
    ...
    this.frameCounter.innerHTML =
      this.frames_rendered + " / " + this.frameCount; //update timer
  ...
  resizeCanvasIfNecessary() {
    ...
    if (
      this.rendering &&
      window.innerWidth / window.innerHeight != this.aspect
    ) {
      this.capturer.stop();
      this.render = null; //hacky way of stopping the rendering
...
function setupThree(fps = 30, length = 5, canvasElem = null) {
  ...
  if (recordString) {
    //detect if URL params include ?record=1 or ?record=true
