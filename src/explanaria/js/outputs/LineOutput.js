...
class LineOutput extends OutputNode {
    ...
    if (Utils.isFunction(options.color)) {
      ...
      this._color =
        options.color !== undefined
          ...
          : new THREE.Color(0x55aa55);
    }

    this.lineJoinType =
      options.lineJoinType !== undefined
        ? options.lineJoinType.toUpperCase()
        : "BEVEL";
  ...
  makeGeometry() {
    ...
    this._nextPointVertices = new Float32Array(
      this._outputDimensions * numVerts
    );
    this._prevPointVertices = new Float32Array(
      this._outputDimensions * numVerts
    );
    ...
    this._geometry.addAttribute(
      "position",
      new THREE.Float32BufferAttribute(this._vertices, this._outputDimensions)
    );
    this._geometry.addAttribute(
      "nextPointPosition",
      new THREE.Float32BufferAttribute(
        this._nextPointVertices,
    ...
    this._geometry.addAttribute(
      ...
      new THREE.Float32BufferAttribute(
        this._prevPointVertices,
        this._outputDimensions
      )
    );
    this._geometry.addAttribute(
      "color",
      new THREE.Float32BufferAttribute(this._colors, 3)
    );
  ...
  _onFirstActivation() {
    ...
    let prevPointPositionAttribute = this._geometry.attributes
      .previousPointPosition;
    this._prevPointVertices = prevVertices;
    prevPointPositionAttribute.setArray(this._prevPointVertices);

    let nextPointPositionAttribute = this._geometry.attributes
      .nextPointPosition;
    ...
    this._geometry.addAttribute(
      "approachNextOrPrevVertex",
      new THREE.Float32BufferAttribute(nextOrPrev, 1)
    );
    ...
    for (
      let vertNum = 0;
      vertNum < this.numCallsPerActivation - 1;
      vertNum += 1
    ) {
      //not sure why this -3 is there. i guess it stops vertNum+3 two lines down from going somewhere it shouldn't?
      let firstCoordinate =
        vertNum % this.itemDimensions[this.itemDimensions.length - 1];
      let endingNewLine =
        firstCoordinate ==
        this.itemDimensions[this.itemDimensions.length - 1] - 1;
  ...
  evaluateSelf(i, t, x, y, z, ...otherArgs) {
    ...
    let firstCoordinate =
      i % this.itemDimensions[this.itemDimensions.length - 1];
    ...
    if (startingNewLine) {
      ...
      let prevX = this._vertices[
        (this._currentPointIndex - 1) * this._outputDimensions * 4
      ];
      let prevY = this._vertices[
        (this._currentPointIndex - 1) * this._outputDimensions * 4 + 1
      ];
      let prevZ = this._vertices[
        (this._currentPointIndex - 1) * this._outputDimensions * 4 + 2
      ];
  ...
  clone() {
    return new LineOutput({
      ...
      lineJoinType: this.lineJoinType,
    });
  }
}
